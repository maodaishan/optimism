...
func NewDerivationPipeline(log log.Logger, cfg *rollup.Config,daCfg *rollup.DAConfig, l1Fetcher L1Fetcher, engine Engine, metrics Metrics, syncCfg *sync.Config) *DerivationPipeline { //DePIN Celestia add daCfg

	// Pull stages
	l1Traversal := NewL1Traversal(log, cfg, l1Fetcher)
	dataSrc := NewDataSourceFactory(log, cfg, daCfg, l1Fetcher) // auxiliary stage for L1Retrieval  //DePIN Celestia add daCfg
